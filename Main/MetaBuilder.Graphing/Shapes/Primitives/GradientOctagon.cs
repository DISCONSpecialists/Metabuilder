...
{
    [Serializable]
    public class GradientOctagon : GoOctagon, ISnappableShape, IIdentifiable, IBehaviourShape
    {
        public GradientOctagon()
        {
        ...
        public override void CopyObjectDelayed(GoCopyDictionary env, GoObject newobj)
        {
            ...
            GradientOctagon octagon = newobj as GradientOctagon;
            octagon.Name = Name.Substring(0, Name.Length);
            if (Manager != null)
                octagon.Manager = Manager.Copy(octagon);
        }
    }
}