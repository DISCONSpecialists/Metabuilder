        ...
        private static class Persistor
        {
            ...
            private static ContentStruct[] LoadContents(XmlTextReader xmlIn)
            {
                ...
                for (int i = 0; i < countOfContents; i++)
                {
                    ...
                    if (contents[i].PersistString.Contains(".MetaPropertyGridDocker"))
                        contents[i].PersistString = contents[i].PersistString.Replace("MetaBuilder.UIControls.GraphingUI", "MetaBuilder.MetaControls");
