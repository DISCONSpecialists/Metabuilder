    ...
    internal class VS2005AutoHideStrip : AutoHideStripBase
    {
        ...
        private void CalculateTabs(DockState dockState)
        {
            ...
            foreach (Pane pane in GetPanes(dockState))
            {
                foreach (TabVS2005 tab in pane.AutoHideTabs)
                {
                    int width = imageWidth + ImageGapLeft + ImageGapRight + TextRenderer.MeasureText(tab.Content.DockHandler.TabText, TextFont).Width + TextGapLeft + TextGapRight;
                    tab.TabX = x;
                    tab.TabWidth = width;
                    x += width;
                }
        ...
        private Rectangle GetTransformedRectangle(DockState dockState, Rectangle rect)
        {
            ...
            return new Rectangle((int)(pts[0].X - (float)rect.Height / 2 + .5F), (int)(pts[0].Y - (float)rect.Width / 2 + .5F), rect.Height, rect.Width);
        }
