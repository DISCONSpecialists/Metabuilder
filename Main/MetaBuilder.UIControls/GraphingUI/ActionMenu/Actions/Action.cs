...
namespace MetaBuilder.UIControls.GraphingUI.ActionMenu.Actions
{
    ...
    internal class ActionData : IDisposable
    {
        ...
        internal void Attach(Action a, Component o, bool designMode)
        {
            ...
            if (_enabled != null && (!_enabled.CanRead || !_enabled.CanWrite) &&
                (_enabled.PropertyType == typeof (bool)))
            {
                // we must be able to read and write a boolean property
                _enabled = null;
            }
            ...
            if (_component is ToolBarButton)
            {
                ...
                Debug.Assert(_checked != null && _checked.CanRead && _checked.CanWrite &&
                             (_checked.PropertyType == typeof (bool)));
            }
            else
            {
                _checked = o.GetType().GetProperty("Checked");
                if (_checked != null && (!_checked.CanRead || !_checked.CanWrite) &&
                    (_checked.PropertyType == typeof (bool)))
                {
                    // we must be able to read and write a boolean property
                    _checked = null;
            ...
            if (_visible != null && (!_visible.CanRead || !_visible.CanWrite) &&
                (_visible.PropertyType == typeof (bool)))
            {
                // we must be able to read and write a boolean property
                _visible = null;
            }
            ...
            if (_shortcut != null && (!_shortcut.CanRead || !_shortcut.CanWrite) &&
                (_enabled.PropertyType == typeof (Shortcut)))
            {
            ...
            if (!(_component is ToolBarButton))
            {
                ...
                if (_imageList != null && (!_imageList.CanRead || !_imageList.CanWrite) &&
                    (_imageList.PropertyType == typeof (ImageList)))
                {
            ...
            if (_imageIndex != null && (!_imageIndex.CanRead || !_imageIndex.CanWrite) &&
                (_imageIndex.PropertyType == typeof (int)))
            {
