    ...
    class SingletonController : MarshalByRefObject
    {
        ...
        private static void CreateInstanceChannel()
        {
            try
            {
                ...
                RemotingConfiguration.RegisterWellKnownServiceType(Type.GetType("MetaBuilder.WinUI.SingletonController"), "SingletonController", WellKnownObjectMode.SingleCall);
            }
            catch
            {
                m_TCPChannel = new TcpChannel(4321);
                ChannelServices.RegisterChannel(m_TCPChannel, false);
                RemotingConfiguration.RegisterWellKnownServiceType(Type.GetType("MetaBuilder.WinUI.SingletonController"), "SingletonController", WellKnownObjectMode.SingleCall);
            }
        }
        ...
        public static void Send(string[] s)
        {
            ...
            catch (Exception exA)
            {
                ...
                try
                {
                    ctrl = (SingletonController)Activator.GetObject(typeof(SingletonController), "tcp://localhost:4321/SingletonController");
                }
                catch (Exception ex)
                {
