...
namespace OpenLicense.LicenseFile.Constraints
{
    ...
    public class DemoConstraint : AbstractConstraint
    {
        ...
        /// belongs to.
        /// </param>
        public DemoConstraint(OpenLicenseFile license)
        {
        ...
        public override void FromXml(XmlNode itemsNode)
        {
            ...
            {
                this.StartDate = MetaBuilder.Core.GlobalParser.ParseGlobalisedDateString(startDateTextNode.Value);
                //this.StartDate	= Convert.ToDateTime( startDateTextNode.Value );
            }

            if (endDateTextNode != null)
            {
                this.EndDate = MetaBuilder.Core.GlobalParser.ParseGlobalisedDateString(endDateTextNode.Value);
            ...
            {
                this.dateLastAccessed = MetaBuilder.Core.GlobalParser.ParseGlobalisedDateString(dateAccessedTextNode.Value);
        ...
        public override string ToXmlString()
        {
            ...
            if (StartDate.Ticks != 0)
                xmlWriter.WriteElementString("StartDate", StartDate.ToString(new CultureInfo("en-ZA")));
            else
                xmlWriter.WriteElementString("StartDate", String.Empty);
            if (EndDate.Ticks != 0)
                xmlWriter.WriteElementString("EndDate", EndDate.ToString(new CultureInfo("en-ZA")));
            else
                xmlWriter.WriteElementString("EndDate", String.Empty);
            xmlWriter.WriteElementString("DateLastAccessed", this.dateLastAccessed.ToString(new CultureInfo("en-ZA")));
        ...
        {
            TimeSpan ts = this.EndDate.Subtract(DateTime.Now);
            if (ts.Days < 0)
            {
                base.License.FailureReason = "Demo Expired";
                return false;
            }
