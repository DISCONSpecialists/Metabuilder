...
namespace MetaBuilder.DataAccessLayer.OldCode.Meta
{
    ...
    public class ObjectAdapter
    {
        ...
        private MetaObject GetObjectForRow(DataRowView drv)
        {
            MetaObject mo = new MetaObject();
            mo.EntityState = EntityState.Changed;
            mo.pkid = int.Parse(drv["pkid"].ToString());
            mo.Machine = drv["Machine"].ToString();
            mo.Class = drv["Class"].ToString();
            mo.VCStatusID = int.Parse(drv["VCStatusID"].ToString());
            mo.VCMachineID = drv["VCMachineID"].ToString();
            mo.UserID = int.Parse(drv["UserID"].ToString());
            mo.WorkspaceName = drv["WorkspaceName"].ToString();
            mo.WorkspaceTypeId = int.Parse(drv["WorkspaceTypeId"].ToString());
            mo.DateCreated = DateTime.Parse(drv["DateCreated"].ToString());
            mo.LastModified = DateTime.Parse(drv["LastModified"].ToString());
            return mo;
        }

        public List<MetaObject> GetObjectsByGraphFileIDMachine(int graphfileid, string machine, string connection)
        {
            List<MetaObject> retval = new List<MetaObject>();
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT     MetaObject.* ");
            sb.Append("FROM         MetaObject ");
            sb.Append("WHERE     ((CAST(pkid AS varchar(50)) + Machine) IN ");
            ...
            sb.Append("WHERE     dbo.GraphFile.pkid = " + graphfileid + " AND dbo.GraphFile.Machine='" + machine + "')) ");
            SqlCommand cmd = new SqlCommand(sb.ToString(), new SqlConnection(connection));
            cmd.CommandType = CommandType.Text;

            SqlDataAdapter dap = new SqlDataAdapter();
            dap.SelectCommand = cmd;
            DataSet ds = new DataSet();
            dap.Fill(ds, "ObjectByMachine");

            DataView dv = ds.Tables[0].DefaultView;
            foreach (DataRowView drv in dv)
            {
                retval.Add(GetObjectForRow(drv));
            }
            return retval;
        }
        public MetaObject GetObjectByPkidMachine(int pkid, string machine, string connection)
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("SELECT     TOP 1 MetaObject.* ");
            sb.Append("FROM         MetaObject ");
            sb.Append("WHERE         pkid = " + pkid + " AND Machine = '" + machine + "'");
            SqlCommand cmd = new SqlCommand(sb.ToString(), new SqlConnection(connection));
            cmd.CommandType = CommandType.Text;
        ...
        public void DeleteObject(int ObjectId, string ObjectMachine, bool useServer)
        {
            ...
            catch (Exception ex)
            {
                ...
                sbAlterQuery.Append("DELETE FROM ARTIFACT WHERE OBJECTID=@OBJECTID AND OBJECTMACHINE=@OBJECTMACHINE" + Environment.NewLine);
        ...
        public void DeleteObject(int ObjectId, string ObjectMachine, bool useServer, bool doNotAlterProcedure)
        {
            string connString = useServer ? Variables.Instance.ServerConnectionString : Variables.Instance.ConnectionString;
            SqlCommand cmd = new SqlCommand("META_DeleteObject", new SqlConnection(connString));
            cmd.CommandType = CommandType.StoredProcedure;
