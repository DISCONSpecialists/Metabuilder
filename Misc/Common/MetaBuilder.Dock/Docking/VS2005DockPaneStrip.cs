    ...
    internal class VS2005DockPaneStrip : DockPaneStripBase
    {
        ...
        private void DrawTab_Document(Graphics g, TabVS2005 tab, Rectangle rect)
        {
            ...
            if (DockPane.ActiveContent == tab.Content)
            {
                ...
                if (tab.Content.DockHandler.TabText.Contains(" *"))
                {
                    startColor = Color.MistyRose;
                    endColor = Color.MistyRose;
                }
        ...
        private void WindowList_Click(object sender, EventArgs e)
        {
            ...
            foreach (TabVS2005 tab in Tabs)
            {
                ...
                try
                {
                    Bitmap icon = tab.Content.GetType().ToString().Contains("Palette") ? content.DockHandler.Icon.ToBitmap() : tab.Content.GetType().ToString().Contains("Lite") ? content.DockHandler.Icon.ToBitmap() : content.DockHandler.DockPanel.ParentForm.Icon.ToBitmap();
                    item = SelectMenu.Items.Add(content.DockHandler.TabText, icon);//content.DockHandler.Icon.ToBitmap()
                }
                catch
                {
                    item = SelectMenu.Items.Add(content.DockHandler.TabText, content.DockHandler.Icon.ToBitmap());
                }
