...
namespace MetaBuilder.UIControls.GraphingUI.ActionMenu
{
    ...
    {
        void OpenFileInApplicableWindow(string fileName, bool skiprefresh);
    }
    ...
    public class MRUManager
    {
        ...
        private int maxDisplayLength = 75; // maximum length of file name for display
        ...
        public void Initialize(Form owner, ToolStripMenuItem mruItem, string regPath)
        {
            ...
            menuItemParent.Click += new EventHandler(OnMRUParentPopup);
            // subscribe to owner form Closing event
            ownerForm.Closing += new CancelEventHandler(OnOwnerClosing);
        ...
        private void OnMRUParentPopup(object sender, EventArgs e)
        {
            ...
            while (myEnumerator.MoveNext())
            {
                ...
                item.Click += new EventHandler(OnMRUClicked);
        ...
        private void OnMRUClicked(object sender, EventArgs e)
        {
            try
            {
                ...
                if (item != null)
                {
                    ...
                    ((IMRUClient)ownerForm).OpenFileInApplicableWindow(filename, false);
                }
            }
            catch (Exception ex)
            {
        ...
        private string GetDisplayName(string fullName)
        {
            ...
            if (fileInfo.DirectoryName == currentDirectory)
                return System.Text.RegularExpressions.Regex.Replace(GetShortDisplayName(fileInfo.Name, maxDisplayLength), "(?<!&)&(?!&)", "&&");
            return System.Text.RegularExpressions.Regex.Replace(GetShortDisplayName(fullName, maxDisplayLength), "(?<!&)&(?!&)", "&&");
        }
