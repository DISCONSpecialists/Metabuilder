...
using MetaBuilder.UIControls.GraphingUI.ActionMenu.Utils;
...
namespace MetaBuilder.UIControls.GraphingUI.ActionMenu.DirBrowser
{
    ...
    public class FolderTree : UserControl
    {
        ...
        private ResourceManager _resman = new ResourceManager("DisconComponents", typeof (FolderTree).Assembly);
        ...
        private class DirectoryComparer : IComparer
        {
            ...
            {
                DirectoryInfo dir1 = (DirectoryInfo) obj1;
                DirectoryInfo dir2 = (DirectoryInfo) obj2;
        ...
        private void FillDrives()
        {
            ...
            foreach (string drive in drives)
            {
                ...
                string name = Win32.GetVolumeName(drive);
                ...
                try
                {
                    FileSystemWatcher dw = new FileSystemWatcher(drive);
                    dw.IncludeSubdirectories = true;
                    dw.NotifyFilter = NotifyFilters.DirectoryName;
                    dw.Created += new FileSystemEventHandler(OnDirCreatedCallback);
                    dw.Deleted += new FileSystemEventHandler(OnDirDeletedCallback);
                    dw.Renamed += new RenamedEventHandler(OnDirRenamedCallback);
            ...
            foreach (FileSystemWatcher fw in _diskWatchers)
            {
        ...
        /// </summary>
        private TreeNode FindNode(string path, TreeNode from, bool createNodes)
        {
            if (path == null || path == "")
            {
                return from;
        ...
        protected override void Dispose(bool disposing)
        {
            if (disposing)
            {
                ...
                foreach (FileSystemWatcher fw in _diskWatchers)
                {
                    ...
                    fw.Created -= new FileSystemEventHandler(OnDirCreatedCallback);
                    fw.Deleted -= new FileSystemEventHandler(OnDirDeletedCallback);
                    fw.Renamed -= new RenamedEventHandler(OnDirRenamedCallback);
                }
                if (components != null)
                    components.Dispose();
            }
        ...
        private void OnDirCreatedCallback(object source, FileSystemEventArgs e)
        {
            BeginInvoke(new FileSystemEventHandler(OnDirCreated), new Object[] {source, e});
        }

        private void OnDirCreated(object source, FileSystemEventArgs e)
        {
        ...
        private void OnDirDeletedCallback(object source, FileSystemEventArgs e)
        {
            BeginInvoke(new FileSystemEventHandler(OnDirDeleted), new Object[] {source, e});
        }

        private void OnDirDeleted(object source, FileSystemEventArgs e)
        {
            // find the closest node
        ...
        private void OnDirRenamedCallback(object source, RenamedEventArgs e)
        {
            BeginInvoke(new RenamedEventHandler(OnDirRenamed), new Object[] {source, e});
        }

        private void OnDirRenamed(object source, RenamedEventArgs e)
        {
            // find the closest node
        ...
        private void OnSelectFolder(object sender, TreeViewEventArgs e)
        {
            if (PathChanged != null)
            {
        ...
        /// </summary>
        private void OnEndEdit(object sender, NodeLabelEditEventArgs e)
        {
