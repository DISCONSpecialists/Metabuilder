...
{
    internal class EntityHasDuplicateSelectorAttributes : BaseRule
    {
        public EntityHasDuplicateSelectorAttributes(string name, ExecutesOnEnum execsOn)
            : base(name, execsOn)
        {
            Description = "Selector Attribute shares name with another. Ensure that Selector Attributes on Subset links (to the same Entity) have unique names.";
        }
        ...
        public override void Apply()
        {
            ...
            foreach (ValidatedItem item in AllItems)
            {
                ...
                if (nodes.ContainsKey(ent.Name))
                {
                    ...
                    {
                        if (n.MBase.pkid != ent.MBase.pkid || n.MBase != ent.MBase)
                        {
                            foundDuplicate = true;
                        }
                    }
