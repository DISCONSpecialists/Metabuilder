...
{
    [Serializable]
    public class GradientTrapezoid : GoTrapezoid, IIdentifiable, IBehaviourShape, ISnappableShape
    {
        public GradientTrapezoid()
        {
        ...
        public override void CopyObjectDelayed(GoCopyDictionary env, GoObject newobj)
        {
            ...
            GradientTrapezoid trapz = newobj as GradientTrapezoid;
            trapz.Name = Name.Substring(0, Name.Length);
            if (Manager != null)
                trapz.Manager = Manager.Copy(trapz);
        }
    }
}