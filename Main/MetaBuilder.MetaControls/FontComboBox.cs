...
{
    public class FontComboBox : ToolStripComboBox
    {
        #region  Private Member Declarations

        public static Dictionary<string, Font> _fontCache;
        ...
        ComboBox Box { get { return (ComboBox)this.Control; } }
        #region  Public Constructors

        public FontComboBox()
        {
            if (FontComboBox._fontCache == null)
                FontComboBox._fontCache = new Dictionary<string, Font>();
        ...
        protected virtual void ClearFontCache()
        {
            if (FontComboBox._fontCache != null)
            {
                foreach (string key in _fontCache.Keys)
                    FontComboBox._fontCache[key].Dispose();
                FontComboBox._fontCache.Clear();
            }
        }
        ...
        protected virtual Font GetFont(string fontFamilyName)
        {
            lock (FontComboBox._fontCache)
            {
                if (!FontComboBox._fontCache.ContainsKey(fontFamilyName))
                {
                    ...
                    FontComboBox._fontCache.Add(fontFamilyName, font);
                }
            }

            return FontComboBox._fontCache[fontFamilyName];
        }
