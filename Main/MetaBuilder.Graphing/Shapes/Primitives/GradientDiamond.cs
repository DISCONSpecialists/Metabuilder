...
{
    [Serializable]
    public class GradientDiamond : GoDiamond, ISnappableShape, IIdentifiable, IBehaviourShape
    {
        public GradientDiamond()
        {
        ...
        public override void CopyObjectDelayed(GoCopyDictionary env, GoObject newobj)
        {
            ...
            GradientDiamond diamond = newobj as GradientDiamond;
            diamond.Name = Name.Substring(0, Name.Length);
            if (Manager != null)
                diamond.Manager = Manager.Copy(diamond);
        }
    }
}